...
def create_maze(height,width):
    ...
    dag = svgwrite.Drawing('Mazes.svg', profile='tiny', size=(int(width)*mm,int(height)*mm))
    length = 0
    Maze = []
    ...
    for x in range(width):
        ...
        for y in range(height):
            ...
            for n in range(6):
                Connections[x][y].append([])
                Connections[x][y][0] = [x + 1,y + 1]
                #for z in range(2):
                    #Connections[x][y][n].append([])
            #Connections[x][y][3][0] = ([y+1])
            #Connections[x][y][3][1] = ([x+1])
    ...
    x_coord = 5
    y_coord = 5
    Connections[x_coord - 1][y_coord - 1][5] = 'Done'
    ...
    while all_connect == False:
        ...
        if not leftvisit or not rightvisit or not upvisit or not downvisit:    
            ...
            if current_x == (x_coord - 1):
                Connections[current_x - 1][current_y - 1][2] = 'East'
                Connections[x_coord - 1][y_coord - 1][4] = 'West'
            if current_x - 1 == x_coord:
                Connections[current_x - 1][current_y - 1][4] = 'West'
                Connections[x_coord - 1][y_coord - 1][2] = 'East'
            if current_y - 1 == y_coord:
                Connections[current_x - 1][current_y - 1][1] = 'North'
                Connections[x_coord - 1][y_coord - 1][3] = 'South'
            if current_y + 1 == y_coord:
                Connections[current_x - 1][current_y - 1][3] = 'South'
                Connections[x_coord - 1][y_coord - 1][1] = 'North'
            Connections[x_coord - 1][y_coord - 1][5] = 'Done'
        '''con = open("Maze Connections.txt",'r')
        con_lines = con.readlines()
        con_count = 0
    ...
    '''for x in Connections:
        for y in x:
            for n in range(4):
                ...
                #dag.add(dag.line(((Connections[x][y])*mm,(current_y+path_width)*mm),((x_coord+path_width)*mm,(y_coord-path_width)*mm),stroke_width=0.01*mm,stroke=svgwrite.rgb(10, 10, 16, '%')))
